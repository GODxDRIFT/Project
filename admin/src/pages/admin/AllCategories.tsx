...
const AllCategories = () => {
  ...
  const handleCheckboxChange = async (e: React.ChangeEvent<HTMLInputElement>, id: string) => {
    ...
    try {
      const res = await postData('change-status', { categoryId: id, statusHome: updatedStatus });
      if (res.success === true) {
        const updated = categories?.map(cat => cat?._id === id ? { ...cat, statusHome: updatedStatus } : cat);
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Show in Home Page</th>
            ...
            {currentCategories.map((category) => (
              <tr key={category?._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <input
                    type="checkbox"
                    checked={category?.statusHome as boolean}
